...
class Autoencoder(torch.nn.Module):
    def __init__(self, nb_f, nb_l, nb_b, fc=None, input_size=256, activations='selu'):
        ...
        def downsampling_block(in_dim, nb_f, nb_l, act):
            layers = []
            for n in range(nb_l):
        ...
        def upsampling_block(in_dim, nb_f, nb_l, act):
        ...
        for n in range(self.nb_b):
            layers += downsampling_block(prev_in, prev_f, self.nb_l, self.activation)
            prev_in = prev_f
            prev_f *= 2
        ...
        if self.fc:
            ...
            layers = build_bottleneck(in_dim, self.fc, self.activation)
        ...
        for n in range(self.nb_b):
            prev_f //= 2
            next_f = prev_f // 2
            layers += upsampling_block(prev_f, next_f, self.nb_l, self.activation)
