...
def test(model, testset, batch_size, directory):
    ...
    hist_n, _ = np.histogram(normal_distribution, bins=1000, range=[normal_distribution.min(), abnormal_distribution.max()])
    hist_a, _ = np.histogram(abnormal_distribution, bins=1000, range=[normal_distribution.min(), abnormal_distribution.max()])
