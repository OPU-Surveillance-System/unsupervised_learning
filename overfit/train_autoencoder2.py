...
import dataset
import overfit.autoencoder2
import utils.metrics
import utils.plot
import utils.process

def train(model, optimizer, trainset, testset, epoch, batch_size, noise_rate, directory):
    """
    ...
    for e in range(epoch):
        ...
        for p in phase:
            ...
            for i_batch, sample in enumerate(tqdm(dataloader)):
                ...
                noise = Variable(torch.rand(inputs.size(0), 3, 256, 256) * noise_rate).float().cuda()
            ...
            if e % 10 == 0:
                ...
                noise_inputs = utils.process.deprocess(noise_inputs)
                noise_inputs = noise_inputs.data.cpu().numpy()
                noise_inputs = np.rollaxis(noise_inputs, 1, 4)
                utils.plot.plot_reconstruction_noiy_images(inputs, pred, noise_inputs, os.path.join(directory, 'example_reconstruction_{}'.format(p), 'epoch_{}.svg'.format(e)))
...
def main(args):
    ...
    #Variables
    ae = overfit.autoencoder2.Autoencoder(3, args.nb_f, args.nb_l, args.nb_b, args.dense)
    ae = ae.cuda()
    print(ae)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-n', dest='noise', type=float, default=0.2, help='Input noise rate')
